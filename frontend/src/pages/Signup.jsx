        ...
        <div className={`min-h-screen h-full transition-colors duration-500 ${darkMode
            ...
            {/* <header className="flex justify-between items-center p-3 sm:p-4">
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                    <button className={`p-2 sm:p-3 rounded-full transition-all duration-300 hover:scale-110 ${darkMode ? 'bg-gray-800 text-white hover:bg-gray-700' : 'bg-white text-gray-700 hover:bg-gray-100'
                        } shadow-lg`}>
                ...
                <div className="flex-1 flex items-center justify-center order-1 lg:order-none mb-8 lg:mb-0">
                    <div className="relative">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <div className={`w-80 h-80 rounded-full transition-all duration-1000 ${darkMode ? 'bg-purple-500/10' : 'bg-purple-200/30'
                                } ${isAnimating ? 'scale-110' : 'scale-100'}`}></div>
                            <div className={`absolute w-60 h-60 rounded-full transition-all duration-1000 delay-200 ${darkMode ? 'bg-pink-500/10' : 'bg-pink-200/30'
                                } ${isAnimating ? 'scale-90' : 'scale-110'}`}></div>
                        </div>

                        {/* Talking Animation Skeleton */}
                        <div className="relative z-10 flex flex-col items-center">
                            {/* Head */}
                            <div className={`w-24 h-24 rounded-full border-4 transition-all duration-500 ${darkMode ? 'border-purple-400' : 'border-purple-500'
                                } ${isAnimating ? 'animate-pulse' : ''}`}>
                                {/* Eyes */}
                                <div className="flex justify-center mt-6 gap-3">
                                    <div className={`w-3 h-3 rounded-full transition-all duration-300 ${darkMode ? 'bg-purple-400' : 'bg-purple-500'
                                        } ${isAnimating ? 'scale-125' : 'scale-100'}`}></div>
                                    <div className={`w-3 h-3 rounded-full transition-all duration-300 ${darkMode ? 'bg-purple-400' : 'bg-purple-500'
                                        } ${isAnimating ? 'scale-125' : 'scale-100'}`}></div>
                                </div>
                                {/* Mouth */}
                                <div className={`mt-2 mx-auto w-6 h-3 border-2 border-t-0 rounded-b-full transition-all duration-300 ${darkMode ? 'border-pink-400' : 'border-pink-500'
                                    } ${isAnimating ? 'scale-110 animate-bounce' : 'scale-100'}`}></div>
                            </div>

                            {/* Neck */}
                            <div className={`w-4 h-8 border-2 border-t-0 transition-all duration-500 ${darkMode ? 'border-purple-400' : 'border-purple-500'
                                }`}></div>

                            {/* Body */}
                            <div className={`w-20 h-24 border-4 rounded-t-3xl transition-all duration-500 ${darkMode ? 'border-purple-400' : 'border-purple-500'
                                }`}></div>

                            {/* Arms */}
                            <div className="absolute top-32 left-0 right-0 flex justify-between px-2">
                                <div className={`w-16 h-4 border-2 rounded-full origin-right transition-all duration-500 ${darkMode ? 'border-purple-400' : 'border-purple-500'
                                    } ${isAnimating ? 'rotate-12' : '-rotate-12'}`}></div>
                                <div className={`w-16 h-4 border-2 rounded-full origin-left transition-all duration-500 ${darkMode ? 'border-purple-400' : 'border-purple-500'
                                    } ${isAnimating ? '-rotate-12' : 'rotate-12'}`}></div>
                            </div>

                            {/* Speech Bubbles */}
                            {/* <div className="items-center">
              <div className="absolute items-center -top-16 -right-8 flex gap-2">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                ...
                <div className="flex-1 flex items-center justify-center order-2 lg:order-none">
                    <div className={`w-full max-w-xl p-4 sm:p-8 rounded-3xl shadow-2xl transition-all duration-500 ${darkMode ? 'bg-gray-800/50 backdrop-blur-lg' : 'bg-white/70 backdrop-blur-lg'
                        }`}>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className={`w-full pl-12 pr-4 py-2 rounded-xl border-2 transition-all duration-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 ${darkMode
                ...
                />
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    onChange={handleInputChange}
                                    className={`w-full pl-12 pr-12 py-2 rounded-xl border-2 transition-all duration-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 ${darkMode
                                        ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400'
                                        : 'bg-white border-gray-200 text-gray-800 placeholder-gray-500'
                                        }`}
                                    required
                            ...
                            {/* Social Sign In */}
                            <button className={`w-full py-3 border-2 rounded-xl font-medium transition-all duration-300 hover:scale-105 ${darkMode
